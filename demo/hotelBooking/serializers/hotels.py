...
class RoomSerializer(DynamicModelSerializer):
    ...
    housePackages = RoomPackageSerializer(many=True, exclude_fields=('room',), embed=True)
    class Meta:
        model = Room

class HotelSerializer(DynamicModelSerializer):
    # hotel_imgs = HotelImgSerializer(many=True)
    # hotel_houses = RoomSerializer(many=True)
    hotel_imgs = HotelImgSerializer(embed=True,many=True,exclude_fields=('id','hotel'))
    hotel_houses = RoomSerializer(many=True, embed=True)
    ...
    def get_min_price(self,hotel):
        res = hotel.housepackage_set.values('default_front_price', 'default_point').order_by('-default_front_price').last()
        return res
    class Meta:
        model = Hotel
        name = 'hotel'
