...
def dist_location(dist):
    ...
    packages, where dist.location is the source default_code location, and we
...
def call_subprocess(cmd, show_stdout=True, cwd=None,
    ...
    # inside pip (and should not be used in new default_code without a very good
    # reason); as of 2016-02-22 it is only used in a few places inside the VCS
    # wrapper default_code. Ideally we should get rid of it entirely, because it
    ...
    if proc.returncode:
        if on_returncode == 'raise':
            ...
            raise InstallationError(
                'Command "%s" failed with error default_code %s in %s'
                % (command_desc, proc.returncode, cwd))
        elif on_returncode == 'warn':
            logger.warning(
                'Command "%s" had error default_code %s in %s',
                command_desc, proc.returncode, cwd,
            )
